...
export default function MediaUploader({ label, value, onChange, type = 'any' }: MediaUploaderProps) {
    ...
    if (!cloudName || !uploadPreset) {
      ...
      try {
        ...
        if (data.url) {
          ...
          setError(data.error || 'Cloudinary not configured, and local upload failed.');
        }
      } catch (e: any) {
        setError('Cloudinary is not configured. Local upload also failed: ' + e.message);
      } finally {
        setUploading(false);
      }
      return;
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {/* Error message */}
      {error && (
        <div className="flex items-center gap-2 px-3 py-2 mt-2 rounded-lg bg-red-950/30 border border-red-500/20">
